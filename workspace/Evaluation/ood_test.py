...
model_name = "CNN_cifar" + str(num_used_classes) if DATA != "cars196" else "effnetb3_" + str(num_used_classes)
...
def split_up_some_classes(x, y):
    ...
    for img, lbl in zip(x, y):
        ...
        if DATA == "cifar10":
            # leave out airplane, bird, frog, horse and ship
    ...
    x_new = tf.reshape(x_new, (-1, 32, 32, 3)) if DATA != "cars196" else tf.reshape(x_new, (-1, 300, 300, 3))
...
ood = tf.reshape(ood, (-1, 32, 32, 3)) if DATA != "cars196" else tf.reshape(ood, (-1, 300, 300, 3))
...
try:
    ...
    model = CNN(classes=num_used_classes) if DATA != "cars196" else build_effnet(num_classes=num_used_classes)
    if DATA == "cifar10":
...
for _ in range(RUNS):
    ...
    for certs, method in zip(certainties, methods):
        ...
        with open('recalls_ood.json') as json_file:
            data = json.load(json_file)
            for i in range(len(thresholds)):
                data[DATA][method][i] = data[DATA][method][i] + [recall[i].item()]
        with open('recalls_ood.json', 'w') as json_file:
...
for i, d in enumerate(data):
    ...
    for m, c in zip(methods, colors):
        if i == 2:
            if m == "Rand SE" or m == "Rand MI" or m == "DataAug SE" or m == "DataAug MI":
                continue
        mean = tf.reduce_mean(data[d][m], axis=-1)
        plt.plot(thresholds[2:], mean[2:], label=m, color=c)

    plt.xlabel("Konfidenz-Schwellenwert")
    plt.ylabel("Recall")
    plt.xticks([0.6, 0.7, 0.8, 0.9, 0.99])
...
plt.subplots_adjust(left=0.06, right=0.84, bottom=0.16, top=0.9, wspace=0.3, hspace=0.35)
