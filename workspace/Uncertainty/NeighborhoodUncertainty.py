...
# (also possible: remove some samples)
...
class NeighborhoodUncertaintyClassifier:
    ...
    def __init__(self, model, xtrain, ytrain, xval, yval, x, path_certainty_model=None, k=3):
        """
        ...
        try:
            self.build_certainty_model()
            self.certainty_model.load_weights(path_certainty_model)
        except:
            self.__train_certainty_model(path_certainty_model)
        self.certainties = self.get_certainties()

    def build_certainty_model(self):
        inp = tf.keras.Input((2*self.k + 1))
        x = tf.keras.layers.Dense(512, activation='relu')(inp)
        x = tf.keras.layers.Dense(256, activation='relu')(x)
        out = tf.keras.layers.Dense(1, activation="sigmoid")(x)

        self.certainty_model = tf.keras.Model(inputs=inp, outputs=out)
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.0004)
        self.certainty_model.compile(optimizer=optimizer,
                                       loss=tf.keras.losses.BinaryCrossentropy(from_logits=False),
                                       metrics=[tf.keras.metrics.BinaryAccuracy()])

    def __get_inputs_certainty_model(self, out, img_batch, train_data: bool):
    ...
    def __get_data_for_certainty_model(self, train: bool):
        x_uncertainty = []
        y_uncertainty = []
        ...
        for _, (img_batch, lbl_batch) in zip(tqdm.tqdm(range(len(dataset))), dataset):
            ...
            inputs = self.__get_inputs_certainty_model(out, img_batch, train)
    ...
    def __train_certainty_model(self, path_certainty_model):
        self.build_certainty_model()

        # prepare datasets for uncertainty model
        xtrain_uncertainty, ytrain_uncertainty = self.__get_data_for_certainty_model(train=True)
        xval_uncertainty, yval_uncertainty = self.__get_data_for_certainty_model(train=False)

        early_stop = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20, restore_best_weights=True)
        rlrop = ReduceLROnPlateau(monitor='val_loss', mode='min', patience=5, factor=0.5, min_lr=1e-6, verbose=1)
        self.certainty_model.fit(xtrain_uncertainty, ytrain_uncertainty,
                                   validation_data=(xval_uncertainty, yval_uncertainty),
                                   callbacks=[early_stop, rlrop],
                                   epochs=10000)
        if path_certainty_model is not None:
            self.certainty_model.save_weights(path_certainty_model)

        loss, acc = self.certainty_model.evaluate(xval_uncertainty, yval_uncertainty, verbose=2)
    ...
    def predict_certainty(self, val_img_batch):
        ...
        inputs = self.__get_inputs_certainty_model(out, val_img_batch, False)
        u = self.certainty_model.predict(inputs, verbose=0)
        return u
